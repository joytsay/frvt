...
void BaseDetection::submitRequest() {
    //slog::info << "BaseDetection::submitRequest triggered"<< slog::endl;
    if (!enabled() || request == nullptr) {/*slog::info << "BaseDetection::submitRequest return triggered"<< slog::endl;*/ return;}
    if (isAsync) {
        //slog::info << "BaseDetection::submitRequest isAsync"<< slog::endl;
        request->StartAsync();
        //slog::info << "BaseDetection::submitRequest StartAsync done"<< slog::endl;
    } else {
        //slog::info << "BaseDetection::submitRequest Infer trigggerd"<< slog::endl;
        request->Infer();
        //slog::info << "BaseDetection::submitRequest Infer done"<< slog::endl;
    }
}
...
void FaceDetection::submitRequest() {
    //slog::info << "Start submitRequest"<< slog::endl;
    if (!enquedFrames){ /*slog::info << "submitRequest enquedFrames false"<< slog::endl;*/ return;}
    enquedFrames = 0;
    resultsFetched = false;
    results.clear();
    //slog::info << "submitRequest results.clear done"<< slog::endl;
    BaseDetection::submitRequest();
    //slog::info << "BaseDetection::submitRequest done"<< slog::endl;
}

void FaceDetection::enqueue(const cv::Mat &frame) {
    //slog::info << "Start enqueue"<< slog::endl;
    if (!enabled()) {/*slog::info << "enqueue return triggered"<< slog::endl;*/ return;}

    if (!request) {
        //slog::info << "enqueue request false"<< slog::endl;
        request = net.CreateInferRequestPtr();
        //slog::info << "enqueue CreateInferRequestPtr done"<< slog::endl;
    }

    width = static_cast<float>(frame.cols);
    height = static_cast<float>(frame.rows);

    //slog::info << "enqueue width:"<<width<< " height:"<<height<< slog::endl;

    Blob::Ptr  inputBlob = request->GetBlob(input);

    //slog::info << "enqueue Blob done"<< slog::endl;

    matU8ToBlob<uint8_t>(frame, inputBlob);

    //slog::info << "enqueue matU8ToBlob done enquedFrames:"<< enquedFrames<< slog::endl;

    //cv::imshow("enqueue frame", frame);
